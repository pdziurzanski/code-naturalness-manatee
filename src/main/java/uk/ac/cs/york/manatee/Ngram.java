...
public class Ngram {
//by Daniel John Benton - https://github.com/DanielJohnBenton/Ngrams.java
	...
	public static ArrayList<ArrayList<String>> ngrams2(ArrayList<String> tokens, int n) {
		if(n <= 1) {
		...
		for(int i = 0; i < c; i++) {
			...
			if((i + n - 1) < c) {
				ArrayList<String> ngramTokens = new ArrayList<String>();
				int stop = i + n;
				ngramTokens.add(tokens.get(i));
				for(int j = i + 1; j < stop; j++) {
					ngramTokens.add(tokens.get(j));
				}
				ngrams.add(ngramTokens);
			}
		}
